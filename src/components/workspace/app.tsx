...
function WorkspaceAppClient({ authRole, authSessions }: WorkspaceAppProps) {
  ...
  const sidebarProviderPanel = (
    <Card
      variant="panel"
      className="min-w-0 p-4 shadow-panel text-white light:text-app-fg"
    >
      <ProviderSelector
        provider={provider}
      ...
      {provider.validationError ? (
        <p className="mt-3 min-w-0 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs wrap-break-word text-red-400 light:border-app-danger-border light:bg-app-danger-bg light:text-app-danger-fg">
          {provider.validationError}
        </p>
      ) : null}
    </Card>
